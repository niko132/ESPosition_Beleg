...
void hwinit() {
	...
	// nokr: both filters are mandatory!!!
	
	ESP_LOGW(TAG, "calling esp_wifi_set_promiscuous_filter");
	wifi_promiscuous_filter_t filer_promi;
	filer_promi.filter_mask = WIFI_PROMIS_FILTER_MASK_ALL;
	esp_wifi_set_promiscuous_filter(&filer_promi);
	ESP_LOGW(TAG, "done esp_wifi_set_promiscuous_filter");
	
	ESP_LOGW(TAG, "calling esp_wifi_set_promiscuous_ctrl_filter");
	wifi_promiscuous_filter_t filer_promi_ctrl;
	filer_promi_ctrl.filter_mask = WIFI_PROMIS_CTRL_FILTER_MASK_ALL;
	esp_wifi_set_promiscuous_ctrl_filter(&filer_promi_ctrl);
	ESP_LOGW(TAG, "done esp_wifi_set_promiscuous_ctrl_filter");
}