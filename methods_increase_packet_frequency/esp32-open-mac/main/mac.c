...
static const uint8_t ap_address[6] = {
	...
	0xC4, 0x2B, 0x44, 0x7D, 0xEE, 0xBB
};

static const uint8_t target_address[6] = {
	0x34, 0x2e, 0xb6, 0x1e, 0xc4, 0x46
};
...
uint8_t to_target_rts_frame_template[] = {
    0xb4, 0x00, 
    0xff, 0x00, // ???
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, // receiver addr
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, // transmitter addr
...
void open_mac_rx_callback(wifi_promiscuous_pkt_t* packet) {
    ...
    if ((memcmp(module_mac_addr, p->receiver_address, 6))
	 && (memcmp(target_address, p->receiver_address, 6))
	 && (memcmp(target_address, p->transmitter_address, 6))
	...
	if (memcmp(target_address, p->receiver_address, 6) == 0) {
		ESP_LOGI(TAG, "MESSAGE TO TARGET");
	}
	
	if (memcmp(target_address, p->transmitter_address, 6) == 0) {
		ESP_LOGI(TAG, "MESSAGE FROM TARGET");
	}
	
	if (p->frame_control.type == IEEE80211_TYPE_CTL) {
		ESP_LOGI(TAG, "RECEIVED CONTROL FRAME");
	}

	if (p->frame_control.type == IEEE80211_TYPE_CTL && p->frame_control.sub_type == IEEE80211_TYPE_CTL_SUBTYPE_RTS) {
		ESP_LOGI(TAG, "RECEIVED RTS FRAME");
	}
	
	if (p->frame_control.type == IEEE80211_TYPE_CTL && p->frame_control.sub_type == IEEE80211_TYPE_CTL_SUBTYPE_CTS) {
		ESP_LOGI(TAG, "RECEIVED CTS FRAME");
	}
...
void build_frames() {
	...
	memcpy(to_target_rts_frame, to_target_rts_frame_template, sizeof(to_target_rts_frame_template));
	// set receiver address
    memcpy(&to_target_rts_frame[4], target_address, 6);
    // set transmitter address
    memcpy(&to_target_rts_frame[10], module_mac_addr, 6);
}
...
void mac_task(void* pvParameters) {
    ...
    while (true) {
        ...
        case IDLE:
            ...
            ESP_LOGI(TAG, "Sending rts frame!");
            tx(to_target_rts_frame, sizeof(to_target_rts_frame));
            break;
        case AUTHENTICATED:
